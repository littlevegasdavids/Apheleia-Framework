        ...
        <div class="grid divide-y divide-solid pt-5 tablet:gap-10 tablet:w-10/12 tablet:mx-auto browser:w-9/12">
            ...
            <div class="pt-5">
                <div class="grid gap-4 grid-cols-2 mb-3">
                    ...
                    <button class="btn btn-primary btn-sm justify-self-end shadow-lg tablet:w-9/12" on:click={()=>window.location.href="/addNewAddress"}>Add New Address<i class="fa-solid fa-plus pl-2"></i></button>
                </div>
            ...
            <div class="grid grid-cols-1 justify-center gap-4 mt-5 pt-5 tablet:mt-0">
                <button class="btn btn-primary shadow-lg tablet:w-8/12 tablet:mx-auto browser:w-6/12" on:click={()=>window.location.href = "/api/customer/logout"}>Logout<i class="fa-solid fa-arrow-right-from-bracket pl-2 fa-lg"></i></button> 
                <button class="btn btn-secondary shadow-lg tablet:w-8/12 tablet:mx-auto browser:w-6/12" on:click={()=>window.location.href = `/customer/changeName/${customer_id}`}>Change Name<i class="fa-solid fa-user-pen fa-lg pl-2"></i></button>
                <button class="btn btn-warning shadow-lg tablet:w-8/12 tablet:mx-auto browser:w-6/12" on:click={()=>window.location.href = `/customer/changePassword/${customer_id}`}>Change Password<i class="fa-solid fa-key fa-lg pl-2"></i></button>
                <button class="btn btn-error shadow-lg tablet:w-8/12 tablet:mx-auto browser:w-6/12" on:click={()=>window.location.href = "/deleteAccount"}>Delete Account<i class="fa-solid fa-circle-minus fa-lg pl-2"></i></button> 
            </div>
